           ...
           if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    //Una creado el usuario, se le asignara por defecto el rol de Usuario Operador
                    string rolOperador = Enum.GetName(typeof(Role), Role.Operador);
                    IdentityResult res = await _userManager.AddToRoleAsync(user, rolOperador);
                    if (res.Succeeded)
                    {
